...
def parse_arguments():
    ...
    args, _ = parser.parse_known_args()  # ← changed

    # Collapse single-element list to scalar so config saves consistently
...
def _normalize_hidden(args):
    ...
    # Handle scalar hidden_size (e.g. -sz 128 collapsed above)
    if isinstance(size, int):
        size = [size] * args.num_layers
